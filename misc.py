...
def get_comparison_mat_python_lst(elements):
    '''
    Computes a < b for cartesian product elements x elements.
    '''
    res = []
...
def main():
    elements = [sint(i) for i in [2, 4, 1, 3]]
    print_list(elements)
    print_mat(get_comparison_mat(elements))
    smallest = argmin(elements)
    print_ln("Smallest index: %s", smallest.reveal())
